...
function loadRegionalSource(rootDir) {
  ...
  const run = (file) => vm.runInContext(readFileSync(path.join(rootDir, file), 'utf8'), context, { filename: file, timeout: 5000 });
  run('nationwide-care-manifest.js');
  const manifest = context.window.NATIONAL_CARE_MANIFEST;
  const rowsByType = {};
  ...
  for (const type of Object.keys(REGIONAL_TYPES)) {
    ...
    if (!entry?.files?.length || !/^\d{4}-\d{2}-\d{2}$/.test(entry.sourceDate)) throw new Error(`지역 페이지 원본 매니페스트 확인 필요: ${type}`);
    context.window.NATIONAL_CARE_DATA = [];
    for (const file of entry.files) run(file);
    const records = Array.from(context.window.NATIONAL_CARE_DATA);
...
function institutionCard(record, page) {
  ...
  const fields = [
    ...
    ...config.staff.map((key) => `<div><dt>${STAFF_LABELS[key]}</dt><dd data-field="${key}">${positiveCount(record[key])}</dd></div>`),
  ];
...
function renderPage(page, plan) {
  ...
  const description = page.city
    ? `${region} ${config.label} ${countText(page.count)}곳의 기관명·주소와 공단 평가등급·평가연도를 확인하세요. 평가 확인 ${countText(page.evaluationCount)}곳. ${page.sourceDate} 시설현황을 기준으로 지역 지도와 기관 비교로 연결합니다.`
    : `${page.province} ${config.label} ${countText(page.count)}곳을 ${countText(districtPages.length)}개 시군구별로 찾으세요. 지역별 기관 수와 공단 평가 확인 수, 전체 기관 목록과 지도를 제공합니다. 시설현황 ${page.sourceDate} 기준.`;
...
<main id="main">
  <section class="search-intro" aria-labelledby="page-title"><div class="container">
    ...
    <ul class="data-summary" aria-label="지역 자료 범위"><li data-summary="count">${config.label} <strong>${countText(page.count)}곳</strong></li><li data-summary="evaluationCount">공단 평가 확인 <strong>${countText(page.evaluationCount)}곳</strong></li><li data-summary="sourceDate">시설현황 <time datetime="${page.sourceDate}">${page.sourceDate.replaceAll('-', '.')}</time> 기준</li></ul>
  </div></section>
...
${districtContent}
    ...
    <section class="source-section" aria-labelledby="source-title"><h2 id="source-title">자료 출처·기준일과 미확인 정보</h2>
      <p>기관명·주소${config.capacity ? '·정원' : ''}·직종별 인력은 <a href="https://www.data.go.kr/data/15124763/fileData.do" target="_blank" rel="noopener">국민건강보험공단 장기요양기관 시설별 현황 (새 창)</a> ${page.sourceDate} 기준 자료입니다. 지역 구분은 해당 자료의 시도·시군구 값을 따릅니다.</p>
      <p>평가등급과 평가연도는 <a href="https://www.data.go.kr/data/15104801/fileData.do" target="_blank" rel="noopener">국민건강보험공단 장기요양기관 평가결과 (새 창)</a>에서 해당 급여의 기관기호로 연결한 공개 평가입니다. 평가연도는 시설현황 기준일과 다르며 기관마다 평가 시기가 다를 수 있습니다.</p>
      <p>평가 미확인은 이 자료에서 해당 기관의 공개 등급 또는 연도를 확인하지 못했다는 뜻이며 낮은 등급을 뜻하지 않습니다. ${config.capacity ? '정원과 ' : ''}인력은 양수로 확인된 자료값만 표시하고, 0 또는 값이 없는 경우 미확인으로 표시합니다. 직종별 인력은 현재 근무 인원이나 담당자 배정을 보장하지 않습니다.</p>
