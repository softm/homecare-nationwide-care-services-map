...
test('225개 기관의 기존 공유 주소를 줄이고 같은 선택·조건·위치를 복원한다', () => {
    const url = new URL('https://homecare.designboard.net/index.html?type=daycare');
...
test('지도 SDK 전에 같은 기록에서 주소만 짧게 바꾸며 기존 history.state를 유지한다', () => {
    const href = 'https://homecare.designboard.net/index.html?share=1&sel=' + sampleIds.join('%2C');
...
test('공유가 아닌 일반 주소와 선택 목록이 없는 주소는 변경하지 않는다', () => {
    for (const href of ['https://homecare.designboard.net/index.html?type=daycare', 'https://homecare.designboard.net/index.html?share=1&lat=37.6&z=10', 'https://homecare.designboard.net/index.html?type=daycare?sel=21126000140']) {
