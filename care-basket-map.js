/** SOFTM-BASKET-MAP START 날짜:20260904 : 검색 결과를 보존한 채 비교함 순서만 지도·경로에 사용하고 늦은 응답의 덮어쓰기를 차단 */
(function (root) {
    'use strict';
    function create(adapter) {
        let snapshot = null, generation = 0, controller = null, ids = [];
        ...
        async function show(rows, { route = false, fit = true } = {}) {
            ...
            adapter.mode(true); adapter.clear(); adapter.select(rows);
            ...
            for (let index = 0; index < rows.length; index += 6) {
                const batch = rows.slice(index, index + 6), coords = await Promise.allSettled(batch.map(row => adapter.geocode(row)));
                if (!current()) return;
            ...
            adapter.status(`비교함 순서대로 ${rows.length}곳의 도로 경로를 계산하고 있습니다.`);
            try {
                const response = await adapter.fetch({ start, goal: points.at(-1), waypoints: points.slice(0, -1), option: 'traoptimal' }, controller.signal);
                if (!current()) return;
                const data = await response.json();
                if (!current()) return;
                const distance = Number(data.summary?.distance ?? data.distance), duration = Number(data.summary?.duration ?? data.duration);
                if (!response.ok || !Array.isArray(data.path) || data.path.length < 2 || data.path.some(p => !Array.isArray(p) || p.length < 2 || !p.every(Number.isFinite)) || !Number.isFinite(distance) || !Number.isFinite(duration)) throw new Error('도로 경로 응답을 확인할 수 없습니다.');
                adapter.draw(data.path);
                adapter.status(`비교함 순서 ${rows.length}곳 · 실제 도로 ${(distance / 1000).toFixed(1)}km · 약 ${Math.round(duration / 60000)}분 · 출발: 기준 위치 → ${rows.map(row => row.n).join(' → ')}`);
            } catch (error) {
                if (current() && error.name !== 'AbortError') adapter.status('도로 경로를 불러오지 못했습니다. 담은 기관과 순서는 유지됩니다. 잠시 후 다시 눌러 주세요.');
            }
        }
