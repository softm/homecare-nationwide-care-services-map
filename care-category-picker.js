...
(function(root){
 ...
 /** SOFTM-TYPE-MENU-TEXT START 날짜:20260911 : 상단에는 생활 상황 문구 대신 기존 기관 카테고리명을 간결하게 표시 */
 const menuLabels=Object.freeze({facility:'요양원·공동생활가정',daycare:'주야간보호','home-care':'방문요양','home-nursing':'방문간호','home-bath':'방문목욕','short-stay':'단기보호','welfare-equipment':'복지용구',dementia:'치매전담형','nursing-hospital':'요양병원(의료기관)'});
 const menuOrder=Object.freeze(['facility','daycare','home-care','home-nursing','home-bath','short-stay','welfare-equipment','dementia','nursing-hospital']);
 const menuLabel=type=>menuLabels[type]||'돌봄 유형';
 ...
 function mount({type,getMap,snapshot=()=>({}),resize=()=>{}}){
  ...
  /** SOFTM-TYPE-MENU-TEXT START 날짜:20260911 : 기존 텍스트형 상단 메뉴로 유형을 바로 선택하고 유형 안내는 별도 유지 */
  ...
  for(const id of menuOrder){
   ...
   const button=document.createElement('button');button.type='button';button.className='care-type-menu-item';button.dataset.typeMenu=id;
   button.textContent=menuLabel(id);
  ...
  const toggle=document.createElement('button');toggle.type='button';toggle.className='care-type-toggle';toggle.textContent='유형 ▾';toggle.setAttribute('aria-label','돌봄 유형 선택');toggle.setAttribute('aria-controls','careTypePanel');toggle.setAttribute('aria-expanded','false');header.append(menu,toggle);document.body.append(header); // SOFTM-TYPE-DEFAULT 날짜:20260914 : 유형 선택 기능임을 짧게 알리고 모바일 상단 공간을 확보
 ...
 root.CareCategoryPicker=Object.freeze({categories,menuOrder,menuLabel,resolve,save,destination,mount});
