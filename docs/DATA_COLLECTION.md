...
<!-- SOFTM-NHIS-PUSH START 날짜:20260917 : 자동 수집 재실행과 동시 코드 반영의 실패 원인을 구분 -->
자동 수집 재실행은 해당 브랜치의 최신 커밋에서 시작한다. 수집·검증 후 반영 직전에 원격 브랜치를 가져와 rebase하며, 푸시 경합은 최대 3회 시도한다. 충돌은 실패로 남기고 강제 푸시하지 않는다. 공공데이터 서버 연결 시간 초과는 수집 단계의 외부 접속 오류이며 Node.js 액션 런타임 경고와 구분한다.
<!-- SOFTM-NHIS-PUSH END -->
