...
- 비교함은 드래그 손잡이로 순서를 바꾸며 유형별 현재 브라우저 세션에 보존한다. 위·아래 이동 버튼은 표시하지 않는다. 이동 중 카드는 포인터를 따라 떠서 움직이고 빈 자리·주변 카드 이동·놓기 강조를 표시한다. 밖에 놓거나 Esc로 취소하면 원래 순서를 유지한다. 모바일은 ‘순서 조정’으로 목록을 펼치고, 키보드는 손잡이에 초점을 둔 뒤 방향키로 조정한다. <!-- SOFTM-DRAG-FEEDBACK 날짜:20260904 : 버튼 없이도 이동과 놓을 위치를 분명하게 인지하도록 유지 -->
- 지도 표시 선택의 ‘비교함에 담은 기관’은 `care-basket-map.js`에서 검색 선택·지도 위치를 보관한 뒤 담은 기관만 표시한다. 이 모드의 지도 이동·확대는 자동검색하지 않으며 ‘검색 결과’ 또는 새 검색으로 복귀한다. 기본 조회 조건과 비교함은 바꾸지 않는다.
...
- 지도 표시 대상은 ‘검색 결과’·‘비교함에 담은 기관’을 이름이 바뀌지 않는 두 선택지로 제공하고 현재 선택을 파란색·체크로 표시한다. ‘경로탐색’ 실행 시 지도로 이동하고 출발지·방문 기관·도로 전체 경로를 포함해 화면 범위를 맞춘다. 진행 중 중복 실행을 막고 서버 실패 사유·응답 지연을 안내하며, 선택된 지도 모드를 다시 눌러도 기존 경로를 지우지 않는다. <!-- SOFTM-ROUTE-VIEW 날짜:20260905 : 버튼 반대 이름과 잘린 경로 때문에 실행 여부를 오해하는 문제를 방지 -->
