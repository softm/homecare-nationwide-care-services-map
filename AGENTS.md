...
<!-- SOFTM-BASKET-ORDER START 날짜:20260904 : 조회 기본조건과 비교함 방문 순서가 다시 혼합되지 않도록 동작을 기록 -->
...
- ‘담은 기관만 지도’는 `care-basket-map.js`에서 검색 선택·지도 위치를 보관한 뒤 담은 기관만 표시한다. 이 모드의 지도 이동·확대는 자동검색하지 않으며 ‘검색 결과 지도’ 또는 새 검색으로 복귀한다. 기본 조회 조건과 비교함은 바꾸지 않는다.
- ‘담은 순서로 경로’는 비교함의 1~16곳을 표시된 순서대로 방문한다. 순서·구성 변경 시 이전 경로와 늦은 응답을 취소하고, 좌표 누락은 기관명으로 안내하며 일부 기관을 임의로 제외해 경로를 계산하지 않는다. 검색 목록 정렬은 비교함 순서와 독립적이다.
