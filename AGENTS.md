...
- 8개 2단계 안내 화면은 제목·지도 찾기·자료 요약 아래에 광고 1개를 둔다. `category-landing-ad-config.js`의 전용 PC 728×90·모바일 320×100 단위를 공용으로 사용하고 인덱스·지도 단위는 재사용하지 않는다. 미발급·미노출 시 제휴 안내를 표시하고 기존 문의 오버레이로 연결한다. <!-- SOFTM-LANDING-ADS 날짜:20260904 : 모든 유형의 동선·광고 위치와 별도 집계 기준을 일치 -->
...
- 통합 `index.html`은 주야간보호 단독 카드를 중복 배치하지 않고 8개 유형을 같은 크기의 카드로 한 번씩 표시한다. 모든 카드는 유형 안내를 거쳐 통합 지도의 해당 `type`으로 연결하고, `index-ad-config.js`의 인덱스 전용 PC 728×90·모바일 320×100 상단 배너를 사용한다. 전국 요양 광고 단위를 재사용하지 않는다. <!-- SOFTM-INDEX-UNIFIED 날짜:20260904 : 유형별 진입을 통일하면서 인덱스 전용 광고의 별도 집계를 유지 -->
