...
- 요양원 검색 대표 `nursing-home-map.html`은 첫 화면의 `지도에서 요양원 찾기`를 주 행동으로 두고 `type=facility` 지도로 연결한다. 이용 순서·다른 기관 유형·출처는 아래에 배치하며 인덱스 설명은 안내 페이지를 거치는 실제 동선과 일치시킨다. 전용 `nursing-home-landing.css`로 다른 유형 소개 화면과 스타일 영향을 분리한다. <!-- SOFTM-NURSING-FLOW 날짜:20260904 : 안내 페이지를 실제 검색 화면으로 오인하거나 설명 버튼과 검색 버튼이 다시 경쟁하지 않도록 진입 기준 고정 -->
