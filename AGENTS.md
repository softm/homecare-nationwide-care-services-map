...
- 지역 검색 페이지는 `scripts/build-regional-seo.mjs`가 원본 매니페스트의 기관 자료로 생성하며, 홈·유형 안내·단일 사이트맵 연결은 `scripts/sync-seo-entrypoints.mjs`가 관리한다. 데이터 재생성 후 `npm run build`와 `npm run check`를 실행한다. 지역 목록은 실제 기관명·주소·평가연도와 해당 지역 지도 필터를 제공하고, 공단 등급을 후기 별점으로 표시하지 않는다. <!-- SOFTM-SEO-REGIONAL 날짜:20260904 : 지역명만 반복하는 검색용 복제 페이지와 데이터 갱신 후 오래된 목록을 방지 -->
