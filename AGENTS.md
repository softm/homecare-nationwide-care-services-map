...
- 홈·서비스 소개 하단에는 `partner-banner.css`의 사업자용 상시 문의 배너를 둔다. 예상 비용은 계산기·결과 밖 하단에 `care-cost-ad-config.js` 전용 PC 728×90·모바일 320×100 광고 한 개를 두고 하단 접근 때만 요청한다. 빈 ID·미노출은 제휴 안내로 전환하며 홈·지도·유형 안내 ID를 재사용하지 않는다. 신규 발급 대상 4개와 기존 ID는 `docs/AD_SETUP.md`를 기준으로 관리한다. 문의에는 배너 위치와 실제 유형을 보내고 홈·소개는 `general`로 구분한다. <!-- SOFTM-AD-PLACEMENTS 날짜:20260904 : 이용자의 검색·계산을 가리지 않으면서 사업자 문의 경로와 광고별 집계 기준을 유지 -->
