...
- 두 지도의 사진 탭·썸네일·확대 등 기존 화면 구성은 유지하고, 전체 수집된 `data/nhis/photos/NN/{기관기호}.json`을 공용 로더에서 직접 읽는다. 사진 표시를 이전 수집목록 캐시로 차단하거나 실시간 조회 방식으로 되돌리지 않는다. <!-- SOFTM-PHOTO-COLLECTED 날짜:20260904 : 사진 수집 완료 후 데이터 연결 변경과 화면 복원을 혼동하지 않도록 기준 고정 -->
