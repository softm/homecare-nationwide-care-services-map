...
(function(root){
 ...
 function mount(config){
  const host=document.getElementById('naverMap'),card=host.closest('.map-card');let active=false,origin,moved;
  ...
  function mapTap(){const action=focusAction(active,config.overlayOpen());if(action==='enter')enter();else if(action==='leave')leave()} // SOFTM-MAP-FOCUS-TOGGLE 날짜:20260914 : 네이버 지도가 확정한 배경 클릭만 열기와 닫기에 사용
  ...
  return {enter,leave,mapTap,active:()=>active}; // SOFTM-MAP-FOCUS-TOGGLE 날짜:20260914 : 지도 SDK 클릭에서 같은 전체보기 전환을 호출
 }
 root.CareMapFocus={mount,focusAction}; // SOFTM-MAP-FOCUS-TOGGLE 날짜:20260914 : 열기·닫기 상태 판정을 회귀검사에서 직접 확인
})(globalThis);
/** SOFTM-MAP-FOCUS END */
