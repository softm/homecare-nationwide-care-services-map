...
담은 기관은 손잡이를 끌어 방문 순서를 바꿉니다. 경로탐색에서는 현재 위치 또는 검색한 주소 후보를 직접 선택해 출발지를 정합니다. 기존 지도 중심은 출발지로 사용하지 않습니다. 1~16곳을 표시된 순서로 방문하고, 출발지·기관·순서가 바뀌면 이전 경로를 취소합니다. 출발지 초안은 현재 페이지 안에서 유지하며 공유 주소에는 넣지 않습니다.
