...
<!-- SOFTM-WORKSPACE START 날짜:20260905 : 두 지도에서 검색과 방문 준비를 같은 흐름으로 인수인계 -->
## 기관 찾기와 방문 준비

두 지도는 **기관 찾기 → 담은 기관 → 비교하기 또는 경로탐색** 흐름을 공유합니다. PC는 왼쪽 목록·오른쪽 지도, 1,000px 이하는 검색창 아래 지도와 스크롤 목록을 함께 표시하고 검색조건 버튼으로 조건을 펼칩니다. 검색 체크와 담은 기관 목록은 별도로 유지하며 탭을 바꾸면 검색 조건·지도 위치·스크롤을 복원합니다.

담은 기관은 손잡이를 끌어 방문 순서를 바꿉니다. 경로탐색에서는 현재 위치 또는 검색한 주소 후보를 직접 선택해 출발지를 정합니다. 기존 지도 중심은 출발지로 사용하지 않습니다. 1~16곳은 기본적으로 표시된 순서로 방문하며, `최적경로 찾기`를 선택하면 출발지와 기관 좌표의 예상 이동거리가 짧은 순서로 배열을 바꾼 뒤 변경 전·후를 알립니다. 바뀐 배열은 담은 기관·지도 번호·도로 경로에 함께 적용합니다. 출발지·기관·순서·최적화 선택이 바뀌면 이전 경로를 취소하며, 출발지 초안은 현재 페이지 안에서 유지하고 공유 주소에는 넣지 않습니다.

공용 상태는 `map-experience.js`, 지도·경로 처리는 `care-basket-map.js`, 출발지 주소 후보는 `naver-geocoder.js`가 담당합니다. 경로 결과는 문구 파싱 없이 상태·거리·시간·출발지·방문 기관으로 전달합니다. `scripts/care-basket.test.mjs`에서 출발지 선택, 검색 상태 보존, 경유지 제한, 응답 취소와 오류를 검사합니다.
<!-- SOFTM-WORKSPACE END -->
...
<!-- SOFTM-ADVANCED-SEARCH START 날짜:20260904 : 상세조건의 데이터 출처와 두 지도의 공통 사용 방법을 안내 -->
## 상세검색

두 지도에서 `상세검색`을 펼치면 현재 급여종류에 해당하는 설립주체·상세 주소·제공서비스 조건을 선택할 수 있습니다. 요양원·공동생활가정은 시설급여 세부유형도 구분합니다. 같은 묶음 안에서는 하나 이상, 다른 묶음끼리는 모두 충족하는 기관을 찾고 선택 즉시 목록과 마커에 적용합니다. 선택 조건은 개별 해제·초기화할 수 있고 공유 링크에도 포함됩니다.

공단 제공서비스 판정은 공개 기관 검색 결과를 정적으로 보존한 `data/nhis/search-index.json.gz`를 사용합니다. 기관명으로 서비스 여부를 추정하지 않습니다. 수집 기준일은 상세검색 안에 표시하며 갱신 방법은 [데이터 수집 문서](docs/DATA_COLLECTION.md)를 참고하세요.
<!-- SOFTM-ADVANCED-SEARCH END -->

<!-- SOFTM-SEARCH-FEEDBACK START 날짜:20260904 : 사용자가 조건 적용과 실제 결과 반영을 구분하도록 안내 -->
조건을 바꾸면 하단 진행바가 표시되고 실제 목록·지도 반영 후 결과 건수를 안내합니다. 상세검색 안내와 대표 건수는 지역·지도·등급 등 현재 조건을 모두 반영한 같은 조회 결과입니다. 페이지나 지도 처리 상한과 전체 조회 수를 구분하며 위치 미확인 기관은 목록에 포함하고 지도 표시 수와 함께 안내합니다. 연속 변경 시 마지막 결과만 알리고, `결과 보기`를 눌렀을 때만 결과 영역으로 이동합니다.
<!-- SOFTM-SEARCH-FEEDBACK END -->

<!-- SOFTM-PHOTO-EXPLORE START 날짜:20260910 : 사진 페이지 사용과 수집 후 요약 갱신 절차를 인수인계 -->
## 사진으로 기관 찾기

`care-photos.html`에서 유형·시도·시군구·기관명으로 사진 등록 기관을 찾고 기존 비교함에 담습니다. 사진 비교는 이 페이지와 두 지도의 담은 기관에서 같은 창으로 열립니다. 공간 분류는 제목 기준이며 사진 등록일을 촬영일로 해석하지 않습니다.

`npm run build`는 `scripts/build_care_photos.py`로 `data/care-photos` 요약 인덱스를 함께 생성합니다. GitHub Actions도 같은 빌드를 사용하므로 수집 뒤 자동 갱신됩니다. `npm run check`는 인덱스와 수집 원본의 일치를 검사합니다. 전체 사진은 기존 기관별 정적 JSON과 공단 이미지 주소를 사용하며 요양병원에는 공단 사진을 적용하지 않습니다.
<!-- SOFTM-PHOTO-EXPLORE END -->

<!-- SOFTM-PHOTO-MAP-SCOPE START 날짜:20260910 : 지도 표시 기관과 사진 검색의 범위 연결을 인수인계 -->
사진 탐색은 지도에서 선택한 현재 표시 마커를 기본 대상으로 사용합니다. 화면 밖·체크 해제 마커는 제외하고, 사진이 없는 기관도 지도 범위 건수에는 포함합니다. 지도 범위 없이 직접 진입하면 먼저 지도를 선택하도록 안내하며 `전체 기관에서 찾기`를 눌러야 전체 탐색으로 전환합니다. 지도에서 사진으로 이동하는 시점의 기관 집합을 세션에 보존하므로 새로고침해도 유지하고, 지도로 돌아가 다시 열면 새 마커 범위를 사용합니다.
