...
통합 시작 화면은 주야간보호를 포함한 8개 기관 유형 안내로 들어가는 링크와 `index-ad-config.js`의 전용 광고 설정을 사용합니다. 광고는 PC `728×90`, 모바일 `320×100` 상단 배너이며 전국 요양 광고 단위와 분리됩니다. <!-- SOFTM-INDEX-AD-UNIT 날짜:20260904 : 승인된 인덱스 광고의 화면별 집계를 요양 지도와 분리 -->

공개 서비스 브랜드는 `돌봄한눈`이며 검색 노출에는 보호자가 사용하는 `요양원`, `주야간보호센터`, `방문요양센터`, `요양병원` 용어를 우선 사용합니다. 검색엔진에는 유형별 정적 소개 페이지를 제출하고 `nationwide-care-services-map.html?type=...` 쿼리 주소는 실제 지도 도구로만 사용합니다. 주야간보호의 검색 대표 페이지는 `daycare-map.html` 안내이며, 실제 검색·비교는 기존 `nationwide-daycare-map.html` 지도에서 실행합니다. <!-- SOFTM-SEO-LANDING 날짜:20260904 : 브랜드·검색 페이지·지도 도구의 역할이 다시 혼재되지 않도록 운영 기준을 기록 -->

8개 안내 화면의 광고는 지도 찾기·자료 요약 다음에 한 개씩 배치합니다. 발급할 광고 규격과 연결 절차는 [`docs/AD_SETUP.md`](docs/AD_SETUP.md)를 참고하세요. <!-- SOFTM-LANDING-ADS 날짜:20260904 : 2단계 전용 광고 발급과 주야간보호의 동일한 진입 흐름을 안내 -->
